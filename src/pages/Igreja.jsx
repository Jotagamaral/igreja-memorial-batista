...
const Igreja = () => {
  ...
  return (
    <div className="min-h-screen py-14 md:py-16 bg-[#F5F5F5]">
      ...
      <div className="max-w-screen-xl mx-auto p-4 md:p-8 flex flex-col md:flex-row gap-8">
        <Card BackgroundColor="#FFFFFF">
          ...
          <div className="p-4 md:p-8 rounded-lg">
            <h1 className={`text-2xl md:text-4xl font-bold font-arya text-left text-custom-blue transition-opacity duration-500 ${!isFading ? 'opacity-100' : 'opacity-0'}`}>
              {texto.name}
            </h1>
            <h3 className={`text-lg md:text-xl font-bold font-poppins text-left text-black transition-opacity duration-500 ${!isFading ? 'opacity-100' : 'opacity-0'}`}>
              {texto.titulo } {/* Mostra título ou role do pastor */}
            </h3>
            <p className={`text-base md:text-lg font-base font-poppins text-justify text-[#191919] mt-4 transition-opacity duration-500 ${!isFading ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="flex flex-col gap-4 md:gap-8 max-w-[700px]">
          ...
          <div className="bg-white p-4 md:p-8 rounded-lg shadow-md">
            <h3 className="text-xl md:text-2xl font-arya font-bold mb-4">Conselho Pastoral</h3>
            <div className="flex flex-wrap justify-center gap-8  mx-auto"> {/* Define uma largura máxima para controlar o número de itens por linha */}
              {igrejaData.conselhoPastoral.map((pastor, index) => (
                ...
                >
                  ...
                  />
                  <p className="font-poppins w-36 h-12">{pastor.name}</p>
                </motion.div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white p-4 md:p-8 rounded-lg shadow-md">
            <h3 className="text-xl md:text-2xl font-arya font-bold mb-4">Líderes das Congregações</h3>
            <div className="flex flex-wrap gap-4 justify-center">
              {igrejaData.lideresCongregacoes.map((lider, index) => (
                <div key={index} className="w-1/2 sm:w-1/3 lg:w-1/5 text-center">
                  ...
                  <p className="font-poppins">{lider.name}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <h1 className="text-3xl md:text-5xl font-arya font-bold text-center mt-8">Conheça as Nossas Congregações e Frentes Missionárias</h1>
      ...
      <div className="text-center py-8 px-4">
        <div className="max-w-screen-lg mx-auto">
          <div className="mb-4">
            <h2 className="text-3xl md:text-4xl font-bold text-custom-blue font-arya">Deseja Conhecer Nossas Congregações?</h2>
          </div>
          <a
