...
const Welcome = ({ cultosRef }) => {
  ...
  const handleScrollToCultos = (e) => {
    ...
    if (location.pathname !== "/") {
      ...
      setTimeout(() => scrollToSection(cultosRef), 1000);
    } else {
  ...
  return (
    <div className='overflow-x-hidden'>
      ...
      >
        <div
          className={`flex flex-col items-center p-4 md:p-10 space-y-4 text-white transition-opacity duration-1000 ${inView ? 'opacity-100' : 'opacity-0'}`}
          ref={ref}
        >
          <ChurchHistory 
